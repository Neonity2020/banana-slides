...
In multi-template mode a **Template** block appears as well, where you can swap this page's template image, write a **Template prompt** for it, or jump straight to **Template Setup** for bulk changes. The panel also shows the page's status, its position in the deck, the number of image versions, and the creation and update timestamps.
